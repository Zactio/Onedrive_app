...
############################################################################
            ...
            search_path = MSGRAPH.get("me/drive/root/search(q='%s')?select=name" % getFileName(fn)[0], headers=request_headers()).data
            file_version = []
            file_named = getFileName(fn)[0]
            file_ext = getFileName(fn)[1]
            for x in search_path["value"]:
                if ((file_ext == x['name'][x['name'].rfind(".")+1:]) and (getFileName(x['name'])[0] == file_named)):#if ext and filename is the same
                    try:
                        file_version = file_version +(getVersion(x['name']))
            ...
            filename = makeFullFileName(file_named, version, file_ext)
