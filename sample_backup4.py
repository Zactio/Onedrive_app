...
class ISODocumentFactory():
    ...
    def createDocument(input_string):
        if isRecord(input_string) == True:
            ...
            return ISORecord(getPartsOfFile(input_string)[0],getPartsOfFile(input_string)[1],getPartsOfFile(input_string)[2],getPartsOfFile(input_string)[3],getPartsOfFile(input_string)[4])
            

        elif isForm(input_string) == True: 
            # return getPartsOfFile(input_string)
            return ISOForm(getPartsOfFile(input_string)[0],getPartsOfFile(input_string)[1],getPartsOfFile(input_string)[2],getPartsOfFile(input_string)[3],getPartsOfFile(input_string)[4])
        else:
...
class ISODocument():
    ...
    def __init__(self, numericId, stringId, version, description, extension):
        self.numericId = numericId[1:]
        self.stringId = stringId
        self.version = version
        self.description = description
        self.extension = extension
        self.filename = numericId+stringId+str(int(version)+1)+description+ '.' + extension
...
def upload():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            InitialDocument = ISODocumentFactory.createDocument(fn)
            ...
            if not documents:
                ...
                firstDocument = sortedDocuments[0]
                fsDoc = sortedDocuments[0][2]
                print (fsDoc)
                InitialDocument.version = fsDoc
                print (InitialDocument.filename) 
